...
##### 1.2.3 inject的规范校验
```provide/inject```这对组合在我们日常开发中使用的可能比较少，当我们需要在父组件中提供数据或者方法给后代组件使用时便需要用到```provide/inject```,注意是后代，而不单纯是子代，这便是有别于```props```的使用场景。它被称为依赖注入，使得组件后代都能访问到注入的数据/方法，且后代不需要知道数据的来源。重要的一点，依赖提供的数据是非响应式的。
