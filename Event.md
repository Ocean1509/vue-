> 这个系列讲到这里，Vue基本核心的东西已经分析完，但是Vue之所以强大，离不开它提供给用户的一些实用功能，开发者可以更偏向于业务逻辑而非基本功能的实现。例如，在我们日常开发中，```@click=***```用得飞起，但是我们是否思考，Vue如何在后面为我们的模板做事件相关的处理，自定义事件的原理和原生dom事件有不同的地方吗，为什么它能够实现父子间的通信，带着疑惑，我们将深入展开源码分析。
### 9.1. 模板编译
